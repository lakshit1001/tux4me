window.onload = function() {
	var game = new Phaser.Game(500, 600, Phaser.CANVAS, "", {
		preload: onPreload,
		create: onCreate
	});
	var sumsArray = [];
	...
	function onPreload() {
		...
		game.load.image("buttonmask", "img/buttonmask.png");
		game.load.spritesheet("buttons", "img/buttons.png",200,50);
		game.load.image("background", "img/bg1.png");
	}
	function onCreate() {
		topScore = localStorage.getItem("topScore")==null?0:localStorage.getItem("topScore");
		background = game.add.tileSprite(0, 0, 500, 600, "background");
		...
		questionText = game.add.text( 260,190,"-",{
			font:"bold 72px Arial blue"
		});
		questionText.anchor.set(0.5);
		scoreText = game.add.text(10,510,"-",{
			font:"bold 24px Arial"
		});
		numberTimer =  game.add.sprite(50,250,"timebar");
		for(var i=0;i<3;i++){
			var numberButton = game.add.button(160,300+i*75,"buttons",checkAnswer,this).frame=i;
		}
